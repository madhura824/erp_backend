...
const addTeacherValidation = async (req, res, next) => {
    // console.log("add teacher val working")
    // next()
    try {
        const { teacherID, name, specialization, designation, DOB,  phoneNo,  email,  password } = req.body;

        if (teacherID == null || name == null || specialization == null || designation == null || DOB == null || phoneNo == null || email == null || password == null) {
        ...
        }   else if (phoneNo < 1000 || phoneNo.length > 999999999999999 ) {
            res.status(400).json({ error: "Phone No must be at least 4 digts long and must not be greater 20", success: false });
