...
const addSubjectValidation = async (req, res, next) => {
    // console.log("addSubjectValidation working ")
    // next()
    try {

        const {subjectID,subjectNameShort,subjectNameLong,subjectType,endTermsFlag,midTermsFlag,activeLearningFlag,theoryAssignmentsFlag,labAssignmentsFlag,maxEndTermMarks,maxMidTermMarks,
        maxActiveLearningMarks,maxTheoryAssignmentMarks,maxLabAssignmentMarks,numberOfLabSessionsPerWeek,numberOfTheorySessionsPerWeek} =req.body;

        if (subjectID == null || subjectNameLong == null || subjectNameShort == null || subjectType == null || numberOfLabSessionsPerWeek == null || numberOfTheorySessionsPerWeek == null || endTermsFlag == null || midTermsFlag == null || activeLearningFlag == null || theoryAssignmentsFlag == null || labAssignmentsFlag == null ) {
        ...
        } else if (subjectID.length < 0 || subjectID.length > 50 ) {
            res.status(400).json({ error: "SubjectID must be between 0 and 50 characters", success: false });
        ...
        console.log(error);
        res.status(500).json({ error: "Internal server error occurred while validating fields while adding the new subject.", success: false });
    }
};

module.exports = addSubjectValidation;
